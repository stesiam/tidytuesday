...
subtitle = glue("Μία από τις χρήσης της τεχνητής νοημοσύνης (ΤΝ) είναι η παραγωγή κειμένου. Παρατηρείται ότι τα μοντέλα GPT-3 και GPT-4 της OpenAI παράγουν
    ...
    **<span style = 'color: #FFDAB9; font-weight:bold;'>{round((df$pct[[2]]- df$pct[[1]]), digits = 1)}</span>**% 
    λιγόυερο πιθανό να αναγωνριστούν σε σύγκριση με το προηγούμενο μοντέλο, GPT-3. Όσον αφορά τα σκάνερ ταξινόμησης κειμένων,
                η <b>OriginalityAI</b> έχει τη μεγαλύτερη ακρίβεια, ωστόσο είναι αρκετά μικρή. Συνεπώς, δεν έχουμε τα εργαλεία
...
p1 = ggplot(df) +
  ...
  geom_richtext(x = 6, y = 70, 
                ...
                fill = NA, label.color = NA, hjust = 0.5) +
  geom_curve(
    aes(x = 4.3, y = 84, xend = 1.5, yend = 80),
    color = "#C74B4B",
    curvature = 0.4,
    size = 0.4,
    arrow = arrow(
